...
Worker receives offer → emits `accepted` receipt to MemoryGate:
...
Worker finishes work → emits `complete` receipt to MemoryGate:
...
AsyncGate detects completion (by polling MemoryGate for receipts) → marks lease as complete → removes from active leases.
...
3. Emit `accepted` receipt to MemoryGate
4. Do work
5. Emit `complete` receipt to MemoryGate
```
...
This state is **not in MemoryGate**. It's coordination machinery.
