...
If MCP auth is enabled (default), you will need an API key for MCP tool calls.
For local development you can temporarily set REQUIRE_MCP_AUTH=false or mint
an API key via the OAuth client credentials flow described in AUTHENTICATION.md.
