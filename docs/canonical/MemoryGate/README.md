...
LegiVellum note: MemoryGate is the knowledge store. The canonical receipt
ledger is ReceiptGate (a MemoryGate profile) and is separate from MemoryGate
by default.
...
MCP Transports and Auth Discovery:
...
- /auth/* - User login + API key management (web UI).
...
- Operational: memorygate.health, memorygate.health_tools, memorygate.health_deps
