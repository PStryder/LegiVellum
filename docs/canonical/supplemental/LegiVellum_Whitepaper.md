...
NOTE (2026-01-26): Supplemental document. ReceiptGate is the canonical receipt
ledger; references to MemoryGate as receipt store are historical. Canonical
contracts in /LegiVellum/docs/canonical/ override.
...
3. **MemoryGate** — durable semantic memory
4. **ReceiptGate** — canonical receipt ledger (MemoryGate profile)
5. **Receipts** — universal proof-of-obligation protocol

### Time & Matter

6. **AsyncGate** — async boundary (time decoupling)
7. **DepotGate** — artifact storage and lifecycle authority

### System Oversight

8. **MetaGate** — bootstrap, topology, and lifecycle authority
...
## MemoryGate: Durable Semantic Memory

MemoryGate is the **authoritative record** of what is known.
...
MemoryGate does not store receipts; it stores knowledge.

State is derived through queries — not maintained imperatively.

---

## ReceiptGate: Canonical Receipt Ledger

ReceiptGate is the **authoritative record** of what has happened.

It stores:

* receipts

ReceiptGate is passive by design.
...
ReceiptGate validates, normalizes, and persists receipts, including those emitted indirectly during bootstrap.
...
AsyncGate passes pointers. ReceiptGate records receipt references; MemoryGate stores knowledge.
...
MetaGate may emit receipts to ReceiptGate on behalf of components during bootstrap windows.
