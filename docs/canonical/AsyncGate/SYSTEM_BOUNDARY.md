...
AsyncGate is MCP-only. All operations are exposed as tools.

### Core Operations (TASKER)
...
- `asyncgate.list_tasks` — List tasks with filters.
- `asyncgate.bootstrap` — Open obligations dump (no bucketing or heuristics).
- `asyncgate.list_receipts` — Query receipt ledger with filters.
- `asyncgate.ack_receipt` — Acknowledge a receipt (append-only receipt).

### Worker Operations (TASKEE)

- `asyncgate.lease_next` — Claim next available tasks.
- `asyncgate.renew_lease` — Extend a lease.
- `asyncgate.report_progress` — Report progress (optional).
- `asyncgate.complete` — Mark task completed.
- `asyncgate.fail` — Mark task failed.

### System

- `asyncgate.health` — Health check.
- `asyncgate.get_config` — Configuration snapshot (optional).
...
- Rate limit fields are returned in MCP response metadata (remaining, reset_at)
- Rate limit violations return structured tool errors with retry hints

**Check Receipt Chain**
```json
{
  "tool": "asyncgate.list_receipts",
  "arguments": {
    "task_id": "{task_id}",
    "limit": 100
  }
