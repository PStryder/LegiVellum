...
AsyncGate no longer exposes a secondary bootstrap facade. MCP-only clients must
use `asyncgate.bootstrap` for open obligations.
...
Check a specific obligation:
```json
{
  "tool": "asyncgate.check_terminator",
  "arguments": {
    "parent_receipt_id": "obligation_receipt_id"
  }
}
```

If `has_terminator` is true, the obligation is discharged.
