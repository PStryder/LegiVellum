...
**Canonical Alignment Note:** ReceiptGate is the canonical receipt ledger. Any
references to "MemoryGate as receipt store/ledger" in this draft should be read
as ReceiptGate (a MemoryGate profile).
...
### 1. MemoryGate - Durable Semantic Memory

**Purpose:** Passive semantic memory store (ReceiptGate provides receipt storage)

**Core Capabilities:**
- Semantic search across observations, patterns, concepts, documents
- Multi-tenant isolation
- Read-only retrieval and promotion of knowledge
...
- Dual-mode operation: standalone or ReceiptGate-integrated
...
**Key Operations:**
- asyncgate.lease_next - Worker polling endpoint (returns 204 or task offer)
...
[MEMORY GAP: Complete MCP tool specifications]
...
- Workers → AsyncGate → ReceiptGate → Domain DeleGate inbox (execution)
- Domain DeleGate escalates → ReceiptGate → Principal inbox (escalations only)
...
**Storage:**
- Append-only ledger in ReceiptGate
...
AsyncGate holds pointers + state
    ↓
ReceiptGate surfaces signal; MemoryGate preserves meaning
    ↓
...
- ReceiptGate receipt store (MemoryGate profile) ✓
...
- Complete MCP tool specifications for most primitives
